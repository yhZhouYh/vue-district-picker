...
(function () {
  ...
  var self_ease = 'cubic-bezier(0.25, 0.8, 0.25, 1)';
  ...
  var AlloyTouch = function (option) {
    ...
    this.forbiden = option.forbiden || false
    ...
    if (this.css) {
      bind(this.eventTarget, endTransitionEventName, this._transitionEndHandler)
    }
    this.eventTarget.addEventListener("touchmove", this._moveHandler, { passive: false, capture: false });
    var _this = this
    // this.eventTarget.addEventListener("touchmove", _throttle(function (e) { _this.pressMove.call(_this, e, _this.target[_this.property]) }, 200));
    this.x1 = this.x2 = this.y1 = this.y2 = null;
  };

  AlloyTouch.prototype = {
    intoView: function (parentbox, el) {
      let parentboxBound = parentbox.getBoundingClientRect()
      let parentScope = this.vertical ? parentboxBound.height : parentboxBound.width
      let elBound = el.getBoundingClientRect()
      let elScope = this.vertical ? elBound.top : elBound.left
      if (elScope <= parentScope) {
        return true
      } else {
        return false
      }
    ...
    _move: function (evt) {
      ...
      if (this.isTouchStart && !this.forbiden) {
        var len = evt.touches.length,
          currentX = evt.touches[0].pageX,
          currentY = evt.touches[0].pageY;

        if (this._firstTouchMove && this.lockDirection) { //锁定方向
    ...
    _to: function (value, time, ease, onChange, onEnd) {
      ...
      if (this.css) {
        ...
        el.style[transitionTimingFunction] = self_ease || ease;
        el[property] = value;
        onChange && onChange.call(self, el[property], this.currentPage);
        this._calculateIndex();
        onEnd()
      } else {
        ...
        var toTick = function () {
          ...
          onChange && onChange.call(self, el[property], self.currentPage);
